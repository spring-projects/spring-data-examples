...
@SpringApplicationConfiguration(classes = ProgressiveTestApplicationConfiguration.class)
public class ProgressiveIntegrationTests {

	@Autowired ProgressiveRepository repository;
	...
	/**
	 * This test demonstrates usage of {@code $comment} {@link Meta} usage. <br />
	 * One can also enable profiling using {@code --profile=2} when starting {@literal mongod}. <br />
	 * <br />
	...
	public void findByFirstnameUsingMetaAttributes() {
		...
		DBCursor cursor = operations.getCollection(ProgressiveTestApplicationConfiguration.SYSTEM_PROFILE_DB).find(
				new BasicDBObject("query.$comment", ProgressiveRepository.META_COMMENT));
